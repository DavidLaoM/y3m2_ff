...
%GLT Teusink
p.GLT.KeqGLT=1;
p.GLT.KmGLTGLCo=10.^x(38).*1.1918; %  [mM]
p.GLT.VmGLT=.2*10.^x(36).*3.67; %--> convert to seconds [mM s-1] 
p.GLT.KmGLTGLCi=10.^x(38).*1.1918; %  [mM]
    
%% FRT 
%FRT Teusink 
p.FRT.KeqFRT=1; 
p.FRT.KmFRTFRCi=10.^x(137).*1.1918;
p.FRT.KmFRTFRCo=10.^x(138).*1.1918;
p.FRT.VmFRT=.2*10.^x(139).*3.67; %--> convert to seconds

%% GLK 
% HXK1 Consensus
p.HXK1_Kadp=10.^x(28).*0.23; %(UNIT!) [mM]
p.HXK1_Katp=10.^x(29).*0.15; %(UNIT!) [mM]
p.HXK1_Keq=10.^x(30).*3800; %(UNIT!) []
p.HXK1_Kg6p=10.^x(31).*30; %(UNIT!) [mM]
p.HXK1_Kglc=10.^x(32).*0.08; %(UNIT!) [mM] (0.1 HXK -> 0.028 GLK)
p.HXK1_Kt6p=10.^x(33).*0.2; %(UNIT!) [mM] (0.2 HXK1, 0.04 HXK2 -> 5 GLK)
p.HXK1_kcat=.2*10.^x(34).*4.75; %(UNIT!) [s-1] 

%% GLK fructose
% HXK1 Consensus
p.HXK1_Keqfrc=10.^x(140).*3800; %(UNIT!)
p.HXK1_Kf6p=10.^x(141).*30; %(UNIT!)
p.HXK1_Kfrc=10.^x(142).*0.08*2; %(UNIT!)
p.HXK1_kcatfrc=.2*10.^x(143).*4.75*1.5; %(UNIT!) 
...
%HOR2 consensus
%p.HOR2_Kglyc3p=5.99.*10.^x(105); %(UNIT!) % david
p.HOR2_Kglyc3p=3.5.*10.^x(105); %(UNIT!) % cronwright
p.HOR2_Kpi=1.*10.^x(106); %(UNIT!)
p.HOR2_kcat=10.^x(107).*0.88; %(UNIT!)
...
p.NTH1_kcat     = 10.^x(123).*100 .* 1.5;%         %(UNIT!) % % changed here (10^(0.54).*.9?) vMax was 15.2

p.UGP_Kutp      = 10.^x(144).* 0.11; %[mM] % Smallbone
p.UGP_Ki_utp    = 10.^x(145).* 0.11; %[mM] % Smallbone
p.UGP_Kg1p      = 10.^x(146).* 0.32; %[mM] % Smallbone
p.UGP_Ki_udp_glc= 10.^x(147).* 0.035; %[mM] % Smallbone
p.UGP_kcat      = 10.^x(148).* 1000; 

p.ATH1_Ktre     = 10.^x(149).* 4.7 ; %.*0.1; %[mM] % (Mittenbühler, K., & Holzer, H. , 1988) % Other value: 1.5 - 4.5 mM François&Parrou 2001
ATH1_vmax       = 99 / 60 / 2e-3; % = 825 % mM/s % (Mittenbühler, K., & Holzer, H. , 1988)
ATH1_kcat       = ATH1_vmax / 0.00196; % = 4.2092e+05 % s-1  % f.ATH1= 0.00196
p.ATH1_kcat   = 10.^x(150).* ATH1_kcat;

MAL_vmax        = 0.140 / 60 / 2e-3; % =1.1667 % mM/s  % 140 - 170 micromol/gX/min (Postma et al. 1990)
% MAL symporter similar to AGT1 (Han et al. 1995)
AGT1_kcat       = MAL_vmax / 0.000067; % =1.7413e+04 s-1   % f.AGT1= 0.000067
...
p.vacuoleT_vmax = 10.^x(155).* 0.6e-3 /60 /0.1;% *10000;  % mM
p.vacuoleT_Ktre = 10.^x(156).* 20 /0.1; % mM 
p.vacuoleT_Keq = 10.^x(157).* 1; % *9; %1000
...
p.mitoPiKm  = 1     .* 10 .^ x(111);
    % temporary to account for the growth rate dependent ATP synthesis
    setup.ratioATP = p.mitoVmax;
    setup.x133 = x(133);

% cytoplasmic ATPase
p.ATPaseK = .12 .* 10 .^ x(129);
    % temporary to account for the growth rate dependent ATP hydrolysis
...
p.vacuolePi_k               = 0.0017    *10.^x(37)  * 100; %500;
% p.vacuolePi_steadyStatePi   = 10        *10.^x(38)  * 1; %*0.7;
p.vacuolePi_steadyStatePi   = 10; %*0.7;

% 2 * ADP = AMP + ATP;  
%ADK1 consensus
p.ADK1_k    = 100   .* 10 .^ x(131);% %(UNIT!)
    % temporary to account for the growth rate dependent ADK1_k
    setup.ratioADK1_k = p.ADK1_k;
    setup.x135 = x(135);
p.ADK1_Keq  = 0.45  .* 10 .^ x(130); %0.275%(UNIT!)
    % temporary to account for the growth rate dependent ADK1_Keq
...
f.UGP = 0.00237; %(mmol/L) From Koen's proteome measurements
f.AGT1= 0.000067; %(mmol/L) From Koen's proteome measurements
f.ATH1= 0.00196; % assuming equal to NTH1
f.ATH1ec = f.ATH1 * 0.9;
f.ATH1vac = f.ATH1 * 0.1;

% glycogen metabolism
p.glycSynthDeg_K = 1 .* 10 .^ x(159);
p.glycSynthDeg_K = 0;
p.glycSynth_K = 1 .* 10 .^ x(160);
p.glycDeg_K = 1 .* 10 .^ x(161);
p.ATH1_Kt6p = 10 .^ x(162) .* 0.1; %(UNIT!)

% ATH1_vac and ATH1_ec
p.ATH1_Ktre_ec = 10 .^ x(163) .* 4.7 ; %.*0.1; %[mM] % (Mittenbühler, K., & Holzer, H. , 1988) % Other value: 1.5 - 4.5 mM François&Parrou 2001
p.ATH1_kcat_ec = 10 .^ x(164) .* ATH1_kcat;
