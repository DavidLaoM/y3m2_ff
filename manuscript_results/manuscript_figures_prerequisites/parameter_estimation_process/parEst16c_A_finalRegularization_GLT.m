...
% %%
simRes1_16c_A = simRes1;
save('tempRes_16c_A_GLT.mat','simRes1_16c_A')
%%
load('tempRes_16c_A_GLT.mat','simRes1_16c_A')


%% Visualizing results: simulation

% visualize (1:8), v_simHXK = ones, w_refY3M1 = increasing
selResCell = simRes1_16c_A([1,2:9]);
plotMode = 2;
referencePlotSimulations_enrichment
plotMode = 0;
...
% 
temp_time = simRes1_16c_A{1}.T_FF01;
...
for i = 1:nPoints
    %
    temp_GLTsim2 = interp1(simRes1_16c_A{i+1}.T_FF01, simRes1_16c_A{i+1}.V_FF01(:,1), temp_timepoints, 'pchip');
    pE16a_error_sim(i) = sum(abs( ( temp_GLTexp - temp_GLTsim2 ) ./ temp_GLTexp ));
    pE16a_error_pars(i) = sum(abs(xAll1(i+1,parsGLT) - x_Y3M1(parsGLT)));
end
...
title('w_{refY3M1} = ones -> w_{GLT} (semilogx)')
...
legend('error simHXK','error parameters')
title('w_{refY3M1} = ones -> w_{GLT} (loglog)')
xlim([min(pE16a_w_sim) max(pE16a_w_sim)])
hold off
...
% suptitle
suptitle('regulatization for 16c A, GLT')
...
for i = 1:nS
    if i == 1
        plot(xAll1(i,parsGLT),'.-','color','k','LineWidth',2)
    else
        plot(xAll1(i,parsGLT),'.-','color',colMap(i,:))
    end
end
plot(x_Y3M1(parsGLT),'.-','color','r','MarkerSize',2)
legend('1','2','3')
% hold off
...
x_test = [xAll1; xAll1; x_Y3M2];
x_test(2,parsGLT) = x_Y3M1(parsGLT);
x_test(3,parsGLT) = x_Y3M1(parsGLT);
for i = 1:2
    xSel = x_test(i,:);
    [T_FF01,Y_FF01,V_FF01] = simulate_FF_enrichment(xSel,canelas_SS,data,dataset,setup,NumberCycles,plotflag,IC);
...
% x16a
xFinal = x16a; 
    xFinal(parsGLT) = xAll1(2,parsGLT); 
    xFinal(parsGLT(1)) = x_Y3M1(parsGLT(1));
% xFinal = xAll1(2,:);

% 
simRes1_test = cell(1,2);
x_test = [x_Y3M2; xFinal];
for i = 1:2
    xSel = x_test(i,:);
    [T_FF01,Y_FF01,V_FF01] = simulate_FF_enrichment(xSel,canelas_SS,data,dataset,setup,NumberCycles,plotflag,IC);
    simRes1_test{i}.T_FF01 = T_FF01;
...
%% final seleciton of parameters
x16c_A_start = x_test(1,:);
x16c_A_final = x_test(2,:);
save('x16c_A_GLT.mat', 'x16c_A_start', 'x16c_A_final')
