...
names_cell = cell(ntests,1);
for i = 1:ntests
    names_cell{i} = sprintf('FF_pE4_x2_wA%d',i);
end
optimOptions2.names_cell = names_cell;
...
for i = 1:ntests
    parComb_cell{i} = [parsHXK,parsPGM1,parsUGP,...
        parsTPS1,parsTPS2,parsNTH1,parsGLY];
end
...
w_idxs1 = [w_g1p, w_udpg, w_t6p_tre, w_treRates_glk_pgi]; % w_idxs1: fit all
w_idxs2 = [w_g1p, w_udpg, w_t6p_tre, w_treRates_glk_pgi]; w_idxs2(end-3) = []; % w_idxs2: fit all except nth1
w_idxs3 = [w_g1p, w_udpg, w_t6p_tre]; % w_idxs3: fit concentrations
w_idxs4 = w_treRates_glk_pgi; % w_idxs4: fit fluxes
w_idxs5_16 = [w_g1p, w_udpg, w_t6p_tre, w_treRates_glk_pgi]; % w_idxs5:16: fit all, with randomness
rng(1), randArr = 1 + (10-1) * rand(12,11);
% 
...
for i = 1:ntests % base,
    ...
    elseif i == 4, warray_cell{i}(w_idxs4) = ones;
        ...
        warray_cell{i}(w_idxs5_16) = randArr(i-4,:); 
    end
end
...
%     setup.w = warray;
    ...
    lb = -3*ones(1,plength); lb(end) = -5;
    ub = 3*ones(1,plength); ub(end) = 5;
...
%     % %%
    ...
    tic
    [xres,resnorm,residual,exitflag] = lsqnonlin(@costfunSystemY3M1_FF_pRun,x_temp,lb,ub,options,canelas_SS,setup,x2,data,dataset,NumberCycles,IC,selPars,warray);
    t = toc; 
    disp(xres);
...
save('pE4a_x2_simRes.mat','simRes1')
%%
load('pE4a_x2_simRes.mat','simRes1')
...
%% screening
for i = 1:15
%     vENO48 = interp1(selResCell{i}.T_FF01, selResCell{i}.V_FF01(:,11), 48, 'pchip');
%     fprintf('i = %f, vENO48 = %f.\n', i, vENO48);
    vUGP35 = interp1(selResCell{i}.T_FF01, selResCell{i}.V_FF01(:,18), 35, 'pchip');
    fprintf('i = %f, vUGP35 = %f.\n', i, vUGP35);
end
% no differences on using w1 and w2, but seeing results of w8, better leave
% it off-fit.
% w3 is the worst (only fitting the concnetrations -> probably not the best)
% w11 (one of the randoms, did not do great either.
% num8, w8 has a better profile for some mets

%%
% select
% selIdxs = [1:3,5:9,11:15];
% selIdxs = [2,3];
% selIdxs = [2,8];
% selIdxs = [1,2];
selIdxs = [1,8];
...
%% probably the case is num.8
x_pE4_x2_start = xAll1(1,:);
x_pE4_x2_end = xAll1(8,:);
% 
save('pset_pE4_x2res.mat','x_pE4_x2_start','x_pE4_x2_end');


% %%
% for i = 1:16
%     fid = fopen('parEst4a_x2_tre_gly_region.m','rt');
%     X = fread(fid);
%     fclose(fid);
%     X = char(X.');
...
%     tempName = sprintf('pE4_x2_%d.m',i);
