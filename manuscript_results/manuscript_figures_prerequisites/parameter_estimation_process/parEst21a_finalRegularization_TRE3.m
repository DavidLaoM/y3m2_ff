...
%% (2021 11 09) latest state parameters and simulations
% Y3M1
load('parSet_99b.mat'); 
x_Y3M1 = x99b;
...
names_cell = cell(ntests,1);
for i = 1:21%1:ntests
    names_cell{i} = sprintf('FF_pE21a_TRE3a_fit2simData_wA%d.mat',i);
end
% 
for i = 22:42%ntests+1:ntests*2
    i2 = i - 21;
    names_cell{i} = sprintf('FF_pE21a_TRE3a_fit2expData_wA%d.mat',i2);
end
% 
optimOptions2.names_cell = names_cell;
...
w_simTRE2 = 63:69;
w_simTRE3a = 63:69;
w_simTRE3b = [21 24 26,55 59 57 58 56]; % G1P UDPGlc T6P,PGM1 TPS1 TPS2 NTH1 UGP
% % w_glc_ic = 6;
% % w_glc_ec = 36;
% % w_g1p = 21;
% % w_udpg = 24;
...
for i = 1:ntests % base,
    ...
    % 
    if i < 22
        warray_cell{i}(w_simTRE3a) = ones * warray_list(i);
    else
        i2 = i - 21;
        warray_cell{i}(w_simTRE3b) = ones * warray_list(i2);
    end
end
% 
setup.regularization_Smallbone2011 = 1;
...
%         t = 1;
    ...
    [simRes_T,simRes_Y,simRes_V] = simulate_FF_enrichment(xres_full,canelas_SS,data,dataset,setup,NumberCycles,plotflag,IC);

    % error simulation
    lam2 = 0;
    [error]=costfunSystemY3M1_FF_pRun_enrichment(xres,canelas_SS,setup,xres_full,data,dataset,NumberCycles,IC,selPars,warray);
    errorSim = sum(abs(error));
    % error parameters
    errorLam = sum(abs(xres));

    % saving result
    parsave_Y3M2_cluster_5(saveName,xres,resnorm,residual,exitflag,t,warray,selPars,simRes_T,simRes_Y,simRes_V, error, errorSim, errorLam)
...
% % % % % 
% % % % errorRes1 = cell(1,length(namesHits1));
% % % % %     w_temp = blankWeight;
% % % % %     w_temp(w_refY3M1) = ones;
% % % % %     w_temp(w_simTRE2) = zeros;
% % % %     w_temp = ones(size(blankWeight));
% % % % selPars = parComb_cell{1};
% % % % setup.selPars = selPars;
% % % % %
...
% % % %     
% % % %     x_temp = xSel(selPars);
% % % %     [error]=costfunSystemY3M1_FF_pRun_enrichment(x_temp,canelas_SS,setup,xSel,data,dataset,NumberCycles,IC,selPars,w_temp);
% % % %     errorRes1{i} = error;
% % % %     % 
...
% % % %        
% % % % %%
% % % % simRes1_16e_TRE = simRes1;
% % % % simRes1_16e_xAll1 = xAll1;
% % % % simRes1_16e_namesHits1 = namesHits1;
% % % % simRes1_16e_errorRes1 = errorRes1;
% % % % 
% % % % save('tempRes_16e_TRE.mat', ...
% % % %     'simRes1_16e_TRE', 'simRes1_16e_xAll1', 'simRes1_16e_namesHits1',...
% % % %     'simRes1_16e_errorRes1')
% % % % %%
% % % % load('tempRes_16e_B_TRE.mat', ...
% % % %     'simRes1_16e_TRE', 'simRes1_16e_xAll1', 'simRes1_16e_namesHits1',...
% % % %     'simRes1_16e_errorRes1')
% % % % 
% % % % 
...
% % % % 
% % % % lambdalist = warray_list; % lambdas
% % % % errorSims = zeros(1,ntests); % errorSims
% % % % errorPars = zeros(1,ntests); % errorPars
% % % % for i = 1:ntests
% % % %     i2 = i+1;
% % % %     errorSims(i) = sum(abs(errorRes1{i2}([299:310,325:end])));
% % % %     errorPars(i) = sum(abs(xAll1(i2,selPars)));
% % % % end
% % % % 
% % % % 
% % % % %% pE16c_A
...
% % % % subplot(2,2,1)
% % % % yyaxis left
% % % % semilogx(lambdalist, errorSims, 'o-')
% % % % hold on
% % % % yyaxis right
% % % % semilogx(lambdalist, errorPars, 'o-')
...
% % % % subplot(2,2,2)
% % % % yyaxis left
% % % % loglog(lambdalist, errorSims, 'o-')
% % % % hold on
% % % % yyaxis right
% % % % loglog(lambdalist, errorPars, 'o-')
% % % % legend('error simTRE','error parameters')
% % % % title('w_{refY3M1} = ones -> w_{TRE} (semilogx)')
% % % % xlim([min(lambdalist) max(lambdalist)])
% % % % hold off
...
% % % % 
% % % % % suptitle
% % % % suptitle('regulatization for 16e, TRE')
% % % % 
% % % % 
% % % % %% Visualizing results: simulation
% % % % % selResCell = simRes1_16e_TRE([1,11:19]);
% % % % selResCell = simRes1_16e_TRE([1,10:13]);
% % % % % selResCell = simRes1_16e_TRE;
% % % % plotMode = 2;
% % % % referencePlotSimulations_enrichment
% % % % plotMode = 0;
...
% % % % plot(xlen,xSel(5,parsTre), 'ko:')
% % % % % 
% % % % xlim([0 15])
% % % % line([0 15],[-1 -1],'Color','black','LineStyle','--')
% % % % line([0 15],[1 1],'Color','black','LineStyle','--')
% % % % % 
% % % % legend('1','10','11','12','13')
% % % % xticks(xlen)
