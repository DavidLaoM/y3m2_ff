...
% %%
simRes1_16c_C = simRes1;
save('tempRes_16c_C_PGM1.mat','simRes1_16c_C')
%%
load('tempRes_16c_C_PGM1.mat','simRes1_16c_C')


%% Visualizing results: simulation

% visualize (1:8), v_simHXK = ones, w_refY3M1 = increasing
selResCell = simRes1_16c_C([1,2:9]);
plotMode = 2;
referencePlotSimulations_enrichment
plotMode = 0;
...
% 
temp_time = simRes1_16c_C{1}.T_FF01;
...
legend('error simPGM1','error parameters')
title('w_{refY3M1} = ones -> w_{PGM1} (semilogx)')
...
legend('error simPGM1','error parameters')
title('w_{refY3M1} = ones -> w_{PGM1} (loglog)')
xlim([min(pE16a_w_sim) max(pE16a_w_sim)])
hold off
...
xlabel('error_{pars}')
ylabel('error_{simPGM1}')
title('roc-like curve')
hold off

% suptitle
suptitle('regulatization for 16c C, PGM1')
