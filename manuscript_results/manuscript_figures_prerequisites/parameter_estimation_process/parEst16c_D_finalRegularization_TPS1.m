...
% %%
simRes1_16c_D = simRes1;
save('tempRes_16c_D_TPS1.mat','simRes1_16c_D')
%%
load('tempRes_16c_D_TPS1.mat','simRes1_16c_D')


%% Visualizing results: simulation

% visualize (1:8), v_simHXK = ones, w_refY3M1 = increasing
selResCell = simRes1_16c_D([1,2:9]);
plotMode = 2;
referencePlotSimulations_enrichment
plotMode = 0;
...
%     temp_PGM1sim2 = interp1(simRes1_16c_C{i+1}.T_FF01, simRes1_16c_C{i+1}.V_FF01(:,17), temp_timepoints, 'pchip');
    temp_TPS1sim2 = interp1(simRes1_16c_D{i+1}.T_FF01, simRes1_16c_D{i+1}.V_FF01(:,17), temp_timepoints, 'pchip');
...
legend('error simTPS1','error parameters')
title('w_{refY3M1} = ones -> w_{TPS1} (semilogx)')
...
legend('error simTPS1','error parameters')
title('w_{refY3M1} = ones -> w_{TPS1} (loglog)')
xlim([min(pE16a_w_sim) max(pE16a_w_sim)])
hold off
...
xlabel('error_{pars}')
ylabel('error_{simTPS1}')
title('roc-like curve')
hold off

% suptitle
suptitle('regulatization for 16c C, TPS1')
