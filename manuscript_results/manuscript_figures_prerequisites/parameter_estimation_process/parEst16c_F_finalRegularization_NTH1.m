...
% %%
simRes1_16c_F = simRes1;
save('tempRes_16c_F_NTH1.mat','simRes1_16c_F')
%%
load('tempRes_16c_F_NTH1.mat','simRes1_16c_F')


%% Visualizing results: simulation

% visualize (1:8), v_simHXK = ones, w_refY3M1 = increasing
selResCell = simRes1_16c_F([1,2:9]);
plotMode = 2;
referencePlotSimulations_enrichment
plotMode = 0;
...
%     temp_TPS2sim2 = interp1(simRes1_16c_F{i+1}.T_FF01, simRes1_16c_F{i+1}.V_FF01(:,17), temp_timepoints, 'pchip');
...
legend('error simNTH1','error parameters')
title('w_{refY3M1} = ones -> w_{NTH1} (semilogx)')
...
legend('error simNTH1','error parameters')
title('w_{refY3M1} = ones -> w_{NTH1} (loglog)')
xlim([min(pE16a_w_sim) max(pE16a_w_sim)])
hold off
...
xlabel('error_{pars}')
ylabel('error_{simNTH1}')
title('roc-like curve')
hold off

% suptitle
suptitle('regulatization for 16c C, NTH1')
