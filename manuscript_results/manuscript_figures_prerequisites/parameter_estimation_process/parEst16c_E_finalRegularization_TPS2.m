...
% %%
simRes1_16c_E = simRes1;
save('tempRes_16c_E_TPS2.mat','simRes1_16c_E')
%%
load('tempRes_16c_E_TPS2.mat','simRes1_16c_E')


%% Visualizing results: simulation

% visualize (1:8), v_simHXK = ones, w_refY3M1 = increasing
selResCell = simRes1_16c_E([1,2:9]);
plotMode = 2;
referencePlotSimulations_enrichment
plotMode = 0;
...
% 
temp_time = simRes1_16c_E{1}.T_FF01;
% temp_glt = simRes1_16c_A{1}.V_FF01(:,1);
% temp_pgm1 = simRes1_16c_C{1}.V_FF01(:,17);
% temp_tps1 = simRes1_16c_E{1}.V_FF01(:,21);
temp_tps2 = simRes1_16c_E{1}.V_FF01(:,19);
% temp_nth1 = simRes1_16c_F{1}.V_FF01(:,20);
...
for i = 1:nPoints
    %
%     temp_PGM1sim2 = interp1(simRes1_16c_C{i+1}.T_FF01, simRes1_16c_C{i+1}.V_FF01(:,17), temp_timepoints, 'pchip');
%     temp_TPS1sim2 = interp1(simRes1_16c_E{i+1}.T_FF01, simRes1_16c_E{i+1}.V_FF01(:,17), temp_timepoints, 'pchip');
    temp_TPS2sim2 = interp1(simRes1_16c_E{i+1}.T_FF01, simRes1_16c_E{i+1}.V_FF01(:,17), temp_timepoints, 'pchip');
%     temp_NTH1sim2 = interp1(simRes1_16c_F{i+1}.T_FF01, simRes1_16c_F{i+1}.V_FF01(:,17), temp_timepoints, 'pchip');
%     pE16a_error_sim(i) = sum(abs( ( temp_PGM1exp - temp_PGM1sim2 ) ./ temp_PGM1exp ));
%     pE16a_error_sim(i) = sum(abs( ( temp_TPS1exp - temp_TPS1sim2 ) ./ temp_TPS1exp ));
    pE16a_error_sim(i) = sum(abs( ( temp_TPS2exp - temp_TPS2sim2 ) ./ temp_TPS2exp ));
%     pE16a_error_sim(i) = sum(abs( ( temp_NTH1exp - temp_NTH1sim2 ) ./ temp_NTH1exp ));
...
%     pE16a_error_pars(i) = sum(abs(xAll1(i+1,parsNTH1) - x_Y3M1(parsNTH1)));
end
...
legend('error simTPS2','error parameters')
title('w_{refY3M1} = ones -> w_{TPS2} (semilogx)')
...
legend('error simTPS2','error parameters')
title('w_{refY3M1} = ones -> w_{TPS2} (loglog)')
xlim([min(pE16a_w_sim) max(pE16a_w_sim)])
hold off
...
xlabel('error_{pars}')
ylabel('error_{simTPS2}')
title('roc-like curve')
hold off

% suptitle
suptitle('regulatization for 16c C, TPS2')


%%
figure
plot(xAll1(1,parsTPS2),'k.-')
hold on
plot(xAll1(3,parsTPS2),'b.-')
plot(x_Y3M1(parsTPS2),'r.-')
legend('x_{Y3M2}','xSel','x_{Y3M1}')


%%
% 
% x16c_A_start = x_test(1,:);
% x16c_A_final = x_test(2,:);
load('x16c_A_GLT.mat', 'x16c_A_start', 'x16c_A_final')
% 
x_test = [x16c_A_final; x16c_A_final];
x_test(2,parsTPS2) = xAll1(3,parsTPS2);
...
%% final seleciton of parameters (x16c_E)
x16c_E_start = x_test(1,:);
x16c_E_final = x_test(2,:);
save('x16c_E_TPS2.mat', 'x16c_E_start', 'x16c_E_final')
